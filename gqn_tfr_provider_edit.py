...
# Later changes made by brettgohre.
...
################################
# Added libraries for fruit still preprocess
import numpy as np
...
def gqn_input_fn(
  ...
  frames, cameras = next_batch() # fruit still dataset batch
  context_frames = frames[:, :-1]
  context_cameras = cameras[:, :-1]
  target = frames[:, -1]
  query_camera = cameras[:, -1]
  context = Context(cameras=context_cameras, frames=context_frames)
  query = Query(context=context, query_camera=query_camera)
  #TaskData(query=query, target=target)?
